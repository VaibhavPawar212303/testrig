import React, { useState } from "react";

const projectID = localStorage.getItem("projectID");

function Timeline({ buildName, buildDate, buildId }) {
  function refreshPage() {
    window.location.reload(false);
  }

  const [selectedBuild, setBuildId] = useState(1);
  localStorage.setItem("Build", selectedBuild);

  return (
    <>
      <div className="grid grid-cols-3 h-4 w-22 ml-96 mr-2 mb-44">
        <ol className="relative border-l border-gray-200 mt-10 ml-20 dark:border-gray-700 float-right col-start-3">
          <li
            className="mb-10 ml-6"
            onClick={() => { 
              refreshPage();
            }}
          >
            <span className="absolute flex items-center justify-center w-3 h-3 bg-blue-100 rounded-full -left-3 ring-8 ring-white dark:ring-gray-900 dark:bg-blue-900">
              <svg
                aria-hidden="true"
                className="w-6 h-6 text-blue-800 dark:text-blue-300"
                fill="currentColor"
                viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"></path>
              </svg>
            </span>
            <h3
              className="flex items-center mb-1 text-sm font-semibold text-gray-900 dark:text-white"
              onClick={(event) => {
                setBuildId(buildId);
              }}
            >
              {buildName}
            </h3>
            <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">
              {buildDate}
            </time>
            <p className="mb-5 text-sm font-normal text-gray-500 dark:text-gray-400">
              Get access to over 20+ pages including a dashboard layout, charts,
              kanban board, calendar, and pre-order E-commerce & Marketing
              pages.
            </p>
          </li>
        </ol>
      </div>
    </>
  );
}

export default Timeline;
